...
function killFred()
        -- Trap information ---
        local npcId = ROOT:GetCustomProperty("ObjectId")
        local npcHealth = ROOT:GetCustomProperty("CurrentHealth")
        local npcCurrentState = ROOT:GetCustomProperty("CurrentState")
		-- print("NPC attack =" .. npcId .. " NPC Health=" .. npcHealth .. " State=" .. npcCurrentState)


		--- Take care of rouge NPC's (freds) which should be dead but were missed due to broadcast conjestion ---		
		if (npcHealth <= 0) then
		    print ("Broadcast a Cleaned up of a rouge Fred current state="..npcCurrentState)

			--Events.Broadcast("ObjectDestroyed", npcId)
			--Events.BroadcastToAllPlayers("ObjectDestroyed", npcId)
			--
			-- don't rebroadcast simply change state to custom property and let Tick() of NCPAIServer do it's thing.
			--local STATE_DEAD_1 = 6
			--local STATE_DEAD_2 = 7
			--local STATE_DISABLED = 8
			--ROOT:SetNetworkedCustomProperty("CurrentState", STATE_DISABLED)

			--- force a destroy here ---
			ROOT:StopMove()
			ROOT:StopRotate()
			ROOT:Destroy()
			
		    print ("Fred Destroy bypass tick() of NPCAISever")
		end
end
